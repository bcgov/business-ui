...
import type { BusinessEntity } from '#business/app/interfaces/business-relationship'
import { fillOutAddress, fillOutName, fillOutNewRelationship, selectDone } from '#business/tests/e2e/test-utils'
...
test.describe('Manage Receivers - Submission', () => {
  ...
  test.describe('Happy paths', () => {
    ...
    test('amendReceiver', async ({ page }) => {
      ...
      expect(requestBody.filing.changeOfReceivers.relationships[0]?.actions).toEqual([ActionType.ADDED])
      expect(requestBody.filing.changeOfReceivers.relationships[1]?.actions).toEqual([ActionType.REMOVED])
      expect(requestBody.filing.changeOfReceivers.relationships[2]?.actions).toEqual([ActionType.NAME_CHANGED])
      expect(requestBody.filing.changeOfReceivers.relationships[3]?.actions).toEqual([ActionType.ADDRESS_CHANGED])
    })
  })
})
