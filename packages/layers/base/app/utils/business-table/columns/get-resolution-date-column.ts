...
export function getResolutionDateColumn<T extends ResolutionDateSchema>(
  ...
  const dateColumn: TableBusinessColumn<T> = {
    id: 'resolution-date',
    meta,
    header: () => h('span', { class: 'sr-only' }, t('label.resolutionOrCourtOrderDate')),
    cell: ({ row }) => {
