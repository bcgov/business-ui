...
test.describe('Business Ledger Tests (document download)', () => {
  ...
  test('Document download displays an error modal if the download fails', async ({ page }) => {
    ...
    await expect(errorModal).toContainText('Toll Free: 1-877-370-1033')
    await expect(errorModal).toContainText('Victoria Office: 1-250-370-1033')
    await expect(errorModal).toContainText('Email: BCRegistries@gov.bc.ca')
