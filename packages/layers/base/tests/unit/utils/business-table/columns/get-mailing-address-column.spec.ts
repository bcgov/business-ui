...
describe('getMailingAddressColumn', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('should have correct properties', () => {
    const column = getMailingAddressColumn() as any
    // should call meta function for meta config
    expect(mockGetColumnMeta).toHaveBeenCalledOnce()

    expect(column.id).toBe('mailingAddress')
    expect(column.header).toBe('Mailing Address')
  ...
  it('cell should have correct properties', () => {
    ...
    expect(cell.type).toBe(TableColumnMailingAddress)
