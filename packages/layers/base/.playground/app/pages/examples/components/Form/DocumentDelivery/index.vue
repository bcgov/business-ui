...
const registeredOfficeEmail = 'reg_office@bcreg.gov.bc.ca'
...
type FullSchema = { docDelivery: DocumentDeliverySchema } & Schema
...
const hasErrors = computed<boolean | undefined>(() => {
  ...
  const folioErrors = docDeliveryRef.value?.formRef?.getErrors()
      ...
      >
        ...
        <div class="p-10 bg-shade">
          <FormDocumentDelivery
            ref="doc-delivery-ref"
            v-model="state.docDelivery"
            name="docDelivery"
            order="X"
            :loading
            :registered-office-email="registeredOfficeEmail"
          />
        </div>
